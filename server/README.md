...
- Expose REST endpoints for dataset marketplace, licensing, transactions, and AI sandboxes.
...
- `src/controllers/`: HTTP request handlers (auth, wallet, dataset, license, purchase, sandbox, token).
- `src/services/`: Business logic layer (sandbox orchestration, AI execution client, access control, file upload, training logs, monitoring).
- `src/models/`: Mongoose document schemas (`Sandbox`, `SandboxFile`, `ExecutionEvent`, `User`, `Dataset`, `License`, `Purchase`, `Transaction`).
...
- `src/jobs/`: Blockchain event indexers and `sandbox-monitor.job.js`.
...
- `npm test`: Run 22 automated backend unit and end-to-end test suites.
